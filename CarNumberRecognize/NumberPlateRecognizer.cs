...
namespace CarNumberRecognize
{
    class NumberPlateRecognizer : DisposableObject
    {
                ...
                using (Mat canny = new Mat()) {
                    using(VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint()) {
                        ...
                        FindLicensePlate(contours, hierachy, 0, gray, canny,
                            licensePlateImageList, filteredLicensePlateImageList, detectedLicensePlateRegionList, licenses);
                    }
                }
            }
            return licenses;
        }
        //Создаем список с распознанными госзнаками в текстовом представлении и список прямоугольниками в которых есть знаки
        void FindLicensePlate(VectorOfVectorOfPoint contours,
            int[,] hierachy,
            int index,
            IInputArray gray,
            IInputArray canny,
        ...
        int GetNumberOfChildren(int[,] hierarchy, int index) {
        ...
        static UMat FilterPlate(UMat plate) {
